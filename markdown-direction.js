...
const DIRECTIONAL_BLOCKS = new Set([
  ...
  "ordered_list_open", "list_item_open", "table_open", "th_open", "td_open"
]);
...
function textInside(tokens, openIndex) {
  ...
  for (let index = openIndex + 1; index < tokens.length; index += 1) {
    ...
    if (token.type === "inline") text += ` ${token.content}`;
  }
  return text;
}
...
function addBlockDirections(markdownIt) {
  markdownIt.core.ruler.after("inline", "persian-block-direction", applyBlockDirections);
  return markdownIt;
}

module.exports = { addBlockDirections, applyBlockDirections };
